...
export class DepotScene extends Phaser.Scene {
  ...
  private renderModeSelect(width: number, height: number): void {
    ...
    /**
     * Each card leads with the thing it is about.
     *
     * It used to lead with a flat colour disc, and the note here conceded
     * what that meant: the hues "only need to be telling apart at a glance".
     * So the most prominent element on every card carried no information,
     * and outweighed the label that carried all of it — a child scanning
     * four cards got four dots and had to read past them.
     *
     * The icons are all art the game already ships. Nothing was
     * commissioned, and nothing needed adding to the manifest: it is
     * generated by scanning `public/assets/icons/`, so every one of these
     * was already loaded and sitting unused.
     *
     * `icon-supply-run` is the van that Parts & Tools is for fixing, and
     * `icon-rescue-centre` is the centre that Decorations brightens — the
     * icon is the *object of the sentence underneath it* rather than a
     * label for the category, which is what makes it readable to someone
     * who cannot yet read the category.
     *
     * The colour stays as the fallback and as the locked state's tint. Two
     * of the four icons are brown, which would have been fatal for discs
     * and does not matter here: a mixing bowl and a van are not confusable
     * whatever colour they are. That is the argument for icons over dots.
     */
    const modes: { mode: DepotMode; colour: number; icon: string; label: string; desc: string }[] = [
      { mode: 'parts_and_tools', colour: hexNum(COLOURS.warm), icon: 'icon-supply-run', label: 'Parts & Tools', desc: 'Fix up the rescue van!' },
      { mode: 'treats_kitchen', colour: hexNum(COLOURS.primaryDark), icon: 'icon-kitchen', label: 'Treats Kitchen', desc: 'Bake tasty treats for animals!' },
      { mode: 'decorations', colour: hexNum(COLOURS.info), icon: 'icon-rescue-centre', label: 'Decorations', desc: 'Brighten up the centre!' },
      { mode: 'medical_supplies', colour: hexNum(COLOURS.accent), icon: 'icon-vet', label: 'Medical Supplies', desc: 'Stock up the vet clinic!' },
    ];
    ...
    modes.forEach((m, i) => {
      ...
      // The mode's own icon, on the disc's mark and at its size. The disc
      // stays as the fallback: a missing texture should degrade to the
      // thing that was there before, not to nothing.
      const iconCx = cardX + 35;
      if (this.textures.exists(m.icon)) {
        this.textures.get(m.icon).setFilter(Phaser.Textures.FilterMode.LINEAR);
        this.container.add(
          this.add.image(iconCx, y, m.icon).setDisplaySize(38, 38).setAlpha(alpha),
        );
      } else {
        this.container.add(
          this.add.circle(iconCx, y, 18, m.colour).setAlpha(alpha),
        );
      }
