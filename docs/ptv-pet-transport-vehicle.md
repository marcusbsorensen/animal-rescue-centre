...
- **Accelerator + brake pedals** — **below the steering wheel**, not off to the side. Left pedal = brake, right pedal = accelerator (real-car layout). **Brake pedal is visibly WIDER than the accelerator** (Marcus, 2026-04-24) — matches real cars where the brake is physically larger so you can't miss it in a panic, and so you can brake with the off-foot. Suggested ratio: brake ~1.6× the accelerator width. Player presses down to engage.
...
### Cockpit slot-map pipeline

The cockpit dashboard layout is interactive-first: we place the
interactive controls (wheel, speedo, ignition, hazard, pedals, GPS,
mirror, cargo-comfort meter) at exact pixel positions on a 1920×1080
canvas, then the painted dashboard art (generated by OpenAI in-paint)
fills the space **around** those slots.

**Pipeline:**

1. **Edit positions** — open `/admin/cockpit-editor.html`. Drag the 11
   interactive-control slots plus the two polyline boundaries
   (`dash-top`, `dash-bottom`) to exact positions. Supports per-vehicle
   tabs (trikey, henry, bea, big-tilly, spark). Auto-saves to
   localStorage; click **Download .json** to export.

2. **Save the JSON** — save the exported file as
...
3. **Live cockpit reads from there** — `/admin/mockup-cockpit.html`
   ...
   the CSS grid fallback until their per-vehicle JSON is designed.

4. **Iterate fast** — the cockpit header has a **Reload layout** button
   that re-fetches the JSON without a full page reload. Edit in the
   editor, save, click reload, see the new layout.

5. **Paint the dashboard backdrop** — export the PNG mask from the
   editor (**Export PNG mask**) and pipe it plus the slots JSON to
   OpenAI in-paint. The mask tells the model which pixels are "hands
   off" (the interactive slots) so the painted dashboard art fills
   only the surrounding background.

**Debug overlay:** add `?showSlots=true` to the cockpit URL to render
`dash-top` and `dash-bottom` polylines as visible lines on top of the
cockpit — useful for checking the painted dashboard's top/bottom
edges line up with the editor's polyline boundaries.
